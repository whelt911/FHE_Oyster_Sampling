...
####Subset only samples taken in fall####
oyster.data <- subset(oyster.data, oyster.data$Date.Collected > "2016-08-01")
quadrat.data <- subset(quadrat.data, quadrat.data$Date.Collected > "2016-08-01")
...
LD.oyster.mean.se$Alive.Dead <- factor(LD.oyster.mean.se$Alive.Dead,
                                       levels = rev(LD.oyster.mean.se$Alive.Dead))
LD.oyster.mean.se <- subset(LD.oyster.mean.se, LD.oyster.mean.se$Mean > 1)
